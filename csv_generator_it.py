...
def tagged_to_synset_it(word):
    try:
        lemmas = wordnet.lemmas(word, lang="ita")
...
def sentence_similarity_fra(sentence, mth):
    ...
    synsets1 = [tagged_to_synset_it(tagged_word) for tagged_word in sentence1]
    synsets2 = [tagged_to_synset_it(tagged_word) for tagged_word in sentence2]
...
def model_gen_train(sentences):
        with open('csv/it/train/sim_it.csv', 'w') as csvfile:
...
def model_gen_test(sentences):
        with open('csv/it/test/sim_it.csv', 'w') as csvfile:
            fieldnames = ['Sentences', 'Score']
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
            writer.writeheader()
            for sentence in sentences:
