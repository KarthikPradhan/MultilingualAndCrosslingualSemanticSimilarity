...
test_corp = ""
with open('csv/fr/train/sim_fr.csv', 'rU') as infile:
...
# Get the sentences and scores from train set
for key in data:
    if key == 'Sentences':
        sent = data[key]
    elif key == 'Score':
...
# get testset
with open('csv/fr/test/sim_fr.csv', 'rU') as infile_t:
    # read the file as a dictionary for each row ({header : value})
...
print("Cosine Similarity of French doc: ", cosine_similarity(vector_1_sent, vector_2_sent))
...
print("LogisticRegression_classifier Algo accuracy:", (nltk.classify.accuracy(LogisticRegression_classifier, test_set))*100)